...
class ProfilePhotoStorageRepository(
    private val contentResolver: ContentResolver,
    private val bucketName: String = "profile-photos"
) {
    ...
    suspend fun uploadProfilePhoto(
        ...
        return try {
            ...
            val bytes = contentResolver.openInputStream(uri)?.use { input ->
                input.readBytes()
            } ?: return Result.failure(Exception("Nao foi possivel abrir a fotografia selecionada."))

            val mimeType = contentResolver.getType(uri)
            val extension = mimeType.toFileExtension()
            val contentType = mimeType.toContentType()
